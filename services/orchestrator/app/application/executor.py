    ...
    def run(self, job_id: str) -> None:
        ...
        with bind_log_context(job_id=job_id):
            ...
            if job.status == JobStatus.aborted.value:
                ...
                logger.info("job already aborted before run", extra={"event": "job.run.skipped"})
                return
            ...
            logger.info(
                ...
                extra={
                    ...
                    "payload_preview": {"skill": job.selected_skill, "input_count": len(input_paths)},
                },
            )
            try:
                self._set_status_or_abort(job_id, JobStatus.running)
                logger.info("opencode session create started", extra={"event": "opencode.session.create.started"})
                ...
                with bind_log_context(session_id=session_id):
                    ...
                    logger.info("opencode session create succeeded", extra={"event": "opencode.session.create.succeeded"})
                    ...
                    logger.info(
                        "opencode prompt async started",
                        extra={
                            "event": "opencode.prompt_async.started",
                            "payload_preview": {"prompt_chars": len(prompt), "agent": ctx.agent, "has_model": bool(ctx.model)},
                        },
                    )
                    ...
                    logger.info("opencode prompt async succeeded", extra={"event": "opencode.prompt_async.succeeded"})
                    ...
                    try:
                        ...
                        logger.error(
                            "opencode last message read failed",
                            extra={
                                "event": "opencode.last_message.read.failed",
                    ...
                    logger.info("job verify started", extra={"event": "job.verify.started"})
                    ...
                    try:
                        ...
                        raise
                    logger.info("job verify succeeded", extra={"event": "job.verify.succeeded"})

                    logger.info("job package started", extra={"event": "job.package.started"})
                    ...
                    logger.info(
                        "job package succeeded",
                        extra={
                            "event": "job.package.succeeded",
                            "payload_preview": {"output_files": len(outputs), "bundle": str(bundle_path.name)},
                        },
                    )
                    logger.info("job run completed", extra={"event": "job.run.completed"})
            except JobAbortedError:
                self._repository.set_status(job_id, JobStatus.aborted)
                logger.info("job run aborted", extra={"event": "job.run.aborted"})
                self._repository.add_event(
                    job_id,
                    source="worker",
        ...
        while time.monotonic() < deadline:
            ...
            try:
                ...
                logger.warning(
                    ...
                    extra={
                        "event": "opencode.event.stream.disconnected",
                        "error_type": type(exc).__name__,
                        "error": str(exc),
                    },
                )
        ...
        *,
        ...
        if current_waiting:
            if approval_wait_started_at is None:
                ...
                logger.info("job entered waiting approval", extra={"event": "job.waiting_approval.enter"})
            ...
            if approval_wait_started_at is not None:
                logger.info("job left waiting approval", extra={"event": "job.waiting_approval.exit"})
